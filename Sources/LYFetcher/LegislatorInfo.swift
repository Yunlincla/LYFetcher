...
/// 立法委員資料抓取基底結構
public struct Legislator: Codable, Hashable, Sendable {
...
/// 抓取最新的立法委員資料
public func FetchAllLegislatorInfo() async throws -> [Legislator] {
    ...
    return try await FetchData(Legislator.self, from: targetURL).dataList.filter { $0.leaveFlag == "否" }
}
